...
set title 'ChestClinic (random)';
set output 'chestclinic-random-compare.ps';
plot 'm=ChestClinic;d=ChestClinic;t=TbOrCa;p=MBrandom;r=uniform;s=normal;b=100;k=10.csv' every 1 using 1:3 lt 1 lc 1 pt 1 lw 4 title 'MBrandom(Bayesian)' with linespoints, \
'm=ChestClinic;d=ChestClinic;t=TbOrCa;p=random;r=uniform;s=normal;b=100;k=10.csv' every 1 using 1:3 lt 2 lc 2 pt 2 lw 4 title 'random(Bayesian)' with linespoints, \
'm=ChestClinic;d=ChestClinic;t=TbOrCa;p=random;r=uniform;s=naive;b=100;k=10.csv' every 1 using 1:3 lt 3 lc 3 pt 3 lw 4 title 'random(na{\357}ve)' with linespoints, \
