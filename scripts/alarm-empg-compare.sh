...
set title 'ALARM (merpg)';
set output 'alarm-empg-compare.ps';
plot 'm=ALARM;d=ALARM;t=Press;p=MBempg;r=uniform;s=normal;b=100;k=10.csv' every 1 using 1:3 lt 1 lc 1 pt 1 lw 4 title 'MBmerpg(Bayesian)' with linespoints, \
'm=ALARM;d=ALARM;t=Press;p=empg;r=uniform;s=normal;b=100;k=10.csv' every 1 using 1:3 lt 2 lc 2 pt 2 lw 4 title 'merpg(Bayesian)' with linespoints, \
'm=ALARM;d=ALARM;t=Press;p=empg;r=uniform;s=naive;b=100;k=10.csv' every 1 using 1:3 lt 3 lc 3 pt 3 lw 4 title 'merpg(na{\357}ve)' with linespoints, \
