...
set title 'Animals (Baseline)';
set output 'animals-bl-compare.ps';
plot 'm=Animals;d=Animals;t=Animal;p=MBbl;r=uniform;s=normal;b=100;k=10.csv' every 1 using 1:3 lt 1 lc 1 pt 1 lw 4 title 'MBbaseline(Bayesian)' with linespoints, \
'm=Animals;d=Animals;t=Animal;p=bl;r=uniform;s=normal;b=100;k=10.csv' every 1 using 1:3 lt 2 lc 2 pt 2 lw 4 title 'Baseline(Bayesian)' with linespoints, \
'm=Animals;d=Animals;t=Animal;p=bl;r=uniform;s=naive;b=100;k=10.csv' every 1 using 1:3 lt 3 lc 3 pt 3 lw 4 title 'Baseline(na{\357}ve)' with linespoints;
EOF
