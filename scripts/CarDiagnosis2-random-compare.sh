...
set title 'CarDiagnosis2 (random)';
set output 'cardiagnosis2-random-compare.ps';
plot 'm=CarDiagnosis2;d=CarDiagnosis2;t=ST;p=MBrandom;r=uniform;s=normal;b=100;k=10.csv' every 1 using 1:3 lt 1 lc 1 pt 1 lw 4 title 'MBrandom(Bayesian)' with linespoints, \
'm=CarDiagnosis2;d=CarDiagnosis2;t=ST;p=random;r=uniform;s=normal;b=100;k=10.csv' every 1 using 1:3 lt 2 lc 2 pt 2 lw 4 title 'random(Bayesian)' with linespoints, \
'm=CarDiagnosis2;d=CarDiagnosis2;t=ST;p=random;r=uniform;s=naive;b=100;k=10.csv' every 1 using 1:3 lt 3 lc 3 pt 3 lw 4 title 'random(na{\357}ve)' with linespoints, \
'm=CarDiagnosis2;d=CarDiagnosis2;t=ST;p=bl;r=uniform;s=normal;b=100;k=10.csv' every 1 using 1:3 lt 4 lc 4 pt 4 lw 4 title 'Baseline(Bayesian)' with linespoints, \
'm=CarDiagnosis2;d=CarDiagnosis2;t=ST;p=bl;r=uniform;s=naive;b=100;k=10.csv' every 1 using 1:3 lt 5 lc 5 pt 5 lw 4 title 'Baseline(na{\357}ve)' with linespoints;
EOF
