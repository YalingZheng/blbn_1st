...
set xrange [0:40];
set ylabel 'Classification Ebror';
set yrange [0:0.65];
...
set title 'ChestClinic (dsepw2)';
set output 'chestclinic-dsepw2-compare.ps';
plot 'm=ChestClinic;d=ChestClinic;t=TbOrCa;p=MBdsepw2;r=uniform;s=normal;b=100;k=10.csv' every 1 using 1:3 lt 1 lc 1 pt 1 lw 4 title 'MBdsepw2(Bayesian)' with linespoints, \
...
'm=ChestClinic;d=ChestClinic;t=TbOrCa;p=dsepw2;r=uniform;s=naive;b=100;k=10.csv' every 1 using 1:3 lt 3 lc 3 pt 3 lw 4 title 'dsepw2(na{\357}ve)' with linespoints, \
'm=ChestClinic;d=ChestClinic;t=TbOrCa;p=bl;r=uniform;s=normal;b=100;k=10.csv' every 1 using 1:3 lt 4 lc 4 pt 4 lw 4 title 'Baseline(Bayesian)' with linespoints, \
'm=ChestClinic;d=ChestClinic;t=TbOrCa;p=bl;r=uniform;s=naive;b=100;k=10.csv' every 1 using 1:3 lt 5 lc 5 pt 5 lw 4 title 'Baseline(na{\357}ve)' with linespoints;
EOF
