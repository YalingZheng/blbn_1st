...
for F in `seq 0 $[ $K - 1 ]`; do
	...
	echo "Executable = /home/choueiry/yzheng/BLBN-EF/blbn_learner" >> $PF_SCRIPT_FILE
	echo "" >> $PF_SCRIPT_FILE
	...
	# program invokation parameters
	args="./blbn_learner -e $NAME -m ./data/$NETWORK/$NETWORK.dne.$STRUCTURE -d ./data/$DATA/$DATA.cas.$F -v ./data/$DATA/$DATA.cas.${F}v -f $F -k $K -b 
$BUDGET -t $TARGET -p $POLICY -s $STRUCTURE -r $PRIOR -o $OUTPUT_REL"
	echo "  OUTPUT:    $args"

	echo "Output = /home/choueiry/yzheng/BLBN-EF/$OUTPUT/debug.stdout.txt.$F" >> $PF_SCRIPT_FILE
	echo "Error  = /home/choueiry/yzheng/BLBN-EF/$OUTPUT/debug.stderr.txt.$F" >> $PF_SCRIPT_FILE
	echo "Log    = /home/choueiry/yzheng/BLBN-EF/$OUTPUT/debug.log.txt.$F"    >> $PF_SCRIPT_FILE
